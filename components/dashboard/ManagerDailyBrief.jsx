...
import { buildAiOperationsBrief, explainDriverRisk } from "../../lib/intelligence/operationsCopilot";
...
export default function ManagerDailyBrief({drivers=[],kpis={},history=[],onOpenDriver,onNavigate}){
  ...
  const aiBrief=buildAiOperationsBrief(drivers,intelligence);
  const evidenceIssues=aiBrief.uniqueDataIssues;
  return <section className="manager-brief">
    ...
    <div className="ai-ops-brief"><div><span className="page-kicker">METRIXIQ AI COPILOT</span><h3>{aiBrief.title}</h3><p>{aiBrief.narrative}</p><small>{aiBrief.guardrail}</small></div></div>
    ...
    <div className="risk-radar">
      ...
      {priorities.length===0?<div className="table-empty-state"><b>No priority intervention</b><span>No driver currently triggers the risk radar.</span></div>:priorities.map((item,index)=>{
        const d=item.driver; const destination=destinationFor(item); const explanation=explainDriverRisk(d);
        return <article key={d.dbId||d.id||index} className="risk-radar-row"><span className="risk-rank">#{index+1}</span><div className="risk-radar-driver"><b>{d.name||"Unresolved identity"}</b><small>{d.site||"No site"} · {item.reasons.slice(0,3).join(" · ")}</small><small><b>AI root cause:</b> {explanation.explanation}</small></div><span className={"risk-pill "+String(d.risk||"low").toLowerCase()}>{d.risk||"Low"}</span><div className="risk-radar-actions"><button onClick={()=>onOpenDriver?.(d)}>Driver 360</button><button onClick={()=>onNavigate?.(destination)}>{destination==="data-quality"?"Data Quality":destination==="evidence"?"Evidence":"Coaching"}</button></div></article>;
      })}
    </div>
  </section>;
}
