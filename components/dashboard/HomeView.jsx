...
export default function HomeView({session,drivers=[],kpis={},history=[],siteFilter="all",sites=[],commandCenter,dataWarning="",onNavigate}){
 ...
 return <div className="homev2">{dataWarning&&<div className="mgrv2-notice error">{dataWarning}</div>}
  ...
  <section className="homev2-grid">
   ...
   <article className="panel homev2-sites"><div className="homev2-title"><h2>Site Performance <small>(Current)</small></h2><button onClick={()=>onNavigate("site-scorecards")}>View Details</button></div><div className="homev2-sitehead"><span>Site</span><span>eMentor</span><span>IADC</span><span>DCR</span><span>FICO</span></div>{siteRows.length===0?<div className="homev2-sites-empty"><b>No sites configured</b><small>Add a site before viewing site-level performance.</small></div>:siteRows.map((site,i)=>{const ds=siteFilter==="all"?drivers.filter(d=>String(d.site||"").trim().toUpperCase()===String(site).trim().toUpperCase()):drivers;return <button className="homev2-siterow" key={site} onClick={()=>onNavigate("site-scorecards")}><b><i className={"dot d"+i}/>{site}</b><span>{metric(mean(ds,"mentor_score","ementor","fico"),"fico")}</span><span>{metric(mean(ds,"iadc"),"iadc")}</span><span>{metric(mean(ds,"dcr"),"dcr")}</span><span>{metric(mean(ds,"fico","mentor_score","ementor"),"fico")}</span></button>})}</article>
  </section>
