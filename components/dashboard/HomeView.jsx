...
export default function HomeView({session,drivers=[],kpis={},history=[],metricRows=[],siteScorecards=[],siteFilter="all",sites=[],commandCenter,dataWarning="",onNavigate}){
 ...
 return <div className="homev2">{dataWarning&&<div className="mgrv2-notice error">{dataWarning}</div>}
  ...
  <section className="homev2-grid">
   ...
   <article className="panel homev2-sites"><div className="homev2-title"><h2>Site Performance <small>(Last Week Scorecard)</small></h2><button onClick={()=>onNavigate("site-scorecards")}>View Details</button></div><div className="homev2-sitehead"><span>Site</span><span>Overall</span><span>Standing</span><span>DSC</span><span>PSB</span></div>{siteRows.length===0?<div className="homev2-sites-empty"><b>No sites configured</b><small>Import a weekly DSP scorecard to populate site performance.</small></div>:siteRows.map((site,i)=>{const cards=siteScorecards.filter(x=>String(x.site||"").toUpperCase()===site).sort((a,b)=>(Number(b.year)-Number(a.year))||(Number(b.week)-Number(a.week)));const card=cards[0]||null;const dsc=safe(card?.metrics?.dsc_dpmo?.value),psb=safe(card?.metrics?.psb?.value);return <button className="homev2-siterow" key={site} onClick={()=>onNavigate("site-scorecards")}><b><i className={"dot d"+i}/>{site}<small>{card?.week_label||""}</small></b><span>{card?.overall_score!=null?Number(card.overall_score).toFixed(2):"—"}</span><span>{card?.standing||"—"}</span><span>{dsc!=null?Math.round(dsc):"—"}</span><span>{psb!=null?psb.toFixed(2):"—"}</span></button>})}</article>
  </section>
