...
export default function WavePlanView({site="DLS2",drivers=[]}){
 ...
 const routeIdentity=useMemo(()=>{const m=new Map();for(const x of routeRows){const route=routeOf(x.cells);if(!route)continue;const key=norm(route),prev=m.get(key)||{trids:[],names:[],fallback:""};const foundTrids=[...new Set(x.cells.flatMap(v=>clean(v).split(/[\\/|,;\s]+/)).filter(v=>/^A[A-Z0-9]{8,}$/i.test(v)).map(norm))];const trids=[...new Set([...prev.trids,...foundTrids])];const names=[...new Set([...prev.names,...trids.map(t=>driverByTrid.get(t)).filter(Boolean)])];const foundName=candidate(x.cells,route,timeOf(x.cells),stageOf(x.cells));m.set(key,{trids,names,fallback:prev.fallback||(!companyLike(foundName)?foundName:"")})}return m},[routeRows,driverByTrid]);
