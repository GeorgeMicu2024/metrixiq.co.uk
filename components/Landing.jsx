...
const plans=[
  ["Free","£0","Explore the workflow",["1 site","10 drivers","CSV / XLSX import","Core dashboard"]],
  ["Pro","£39","For one operating team",["3 sites","150 drivers","PDF review","Risk & coaching"]],
  ["Business","£89","For growing DSP operations",["10 sites","500 drivers","Advanced intelligence","Management reports"],true],
  ["Full","£169","For multi-site operators",["Unlimited sites","Predictive insights","Owner controls","Priority support"]],
];
...
function ProductPreview(){
  return <div className="product-window">
    ...
    <div className="window-body">
      <aside className="preview-side"><b>Overview</b><span>Drivers</span><span>Performance</span><span>Coaching</span><span>AI Insights</span><span>Imports</span></aside>
...
export default function Landing(){
  return <main className="marketing">
    ...
    <section id="pricing" className="pricing-section"><div className="pricing-intro"><span className="section-kicker">PRICING</span><h2>Start small. Add intelligence as the operation grows.</h2></div><div className="plans">{plans.map(([name,price,desc,fs,featured])=><article className={featured?"plan featured":"plan"} key={name}>{featured&&<span className="plan-tag">Most popular</span>}<h3>{name}</h3><p>{desc}</p><div className="price">{price}<small>/month</small></div><ul>{fs.map(f=><li key={f}>✓ {f}</li>)}</ul><Link href="/login?mode=register" className={featured?"primary-btn full":"secondary-btn full"}>Start free</Link></article>)}</div></section>
