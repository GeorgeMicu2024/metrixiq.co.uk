...
const dwcOf=r=>n(r.raw_data?.dwc);\nconst METRIC_META={iadc:{label:"IADC",title:"IADC & DWC — Driver Compliance",kicker:"WORKFLOW COMPLIANCE",description:"In-App Delivery Compliance (IADC) and Driver Workflow Compliance (DWC). Analyse performance, trends and error breakdowns."},pod:{label:"POD",title:"POD — Photo-on-Delivery Compliance",kicker:"DELIVERY QUALITY",description:"Photo-on-Delivery compliance by driver. Weekly values come from scorecards; Daily values are shown only when a true daily source exists."},dcr:{label:"DCR",title:"DCR — Delivery Completion Rate",kicker:"DELIVERY PERFORMANCE",description:"Delivery Completion Rate by driver. Weekly values come from scorecards; Daily values are shown only when a true daily source exists."},cc:{label:"CC",title:"Customer Compliance",kicker:"CUSTOMER COMPLIANCE",description:"Customer Compliance by driver. Weekly values come from scorecards; Daily values are shown only when a true daily source exists."}};
...
export default function IadcView({organizationId,onOpenDriver,onImport,siteFilter="all"}){
  ...
  const counts={excellent:selected.filter(r=>Number(metricValue(r))>=90).length,target:selected.filter(r=>Number(metricValue(r))>=80&&Number(metricValue(r))<90).length,risk:selected.filter(r=>Number(metricValue(r))>=70&&Number(metricValue(r))<80).length,critical:selected.filter(r=>Number(metricValue(r))<70).length};
  const filtered=selected.filter(r=>{const v=Number(metricValue(r));return(bandFilter==="all"||band(v)===bandFilter)&&`${dname(r.drivers)} ${trid(r.drivers)}`.toLowerCase().includes(query.toLowerCase())});
  ...
  return <div className="iadcv3">
    ...
    <section className="iadcv3-kpis">
      ...
      <article><i>◫</i><div><span>{meta.label} (Average)</span><strong>{pct(avg,1)}</strong><small>Target ≥ {TARGETS.iadc}%</small></div></article>
