...
export default function IadcView({organizationId,onOpenDriver,onImport,siteFilter="all"}){
  ...
  return <div className="iadcv3">
    ...
    <section className="iadcv3-lower">
      <article className="panel iadcv3-table"><div className="panel-head"><div><h2>Driver Performance</h2><p>Click a driver to view detailed breakdown</p></div></div><div className="table-wrap"><table><thead><tr><th>#</th><th>Driver name</th><th>Transporter ID</th><th>{meta.label} %</th><th>Band</th><th>DWC %</th><th>Actions</th></tr></thead><tbody>{shown.map((r,i)=>{const v=Number(metricValue(r)),b=metricBand(v);return <tr key={r.driver_id+"-"+i} className={active===r?"active":""} onClick={()=>setDetail(r)}><td><span className="rank-badge">{i+1}</span></td><td><b>{dname(r.drivers)}</b></td><td><code>{trid(r.drivers)}</code></td><td><strong className={"iadcv3-score "+b}>{pct(v)}</strong></td><td><span className={"iadcv3-band "+b}>{metricBandLabel(v)}</span></td><td><b>{pct(dwcOf(r))}</b></td><td><button onClick={e=>{e.stopPropagation();setDetail(r)}}>◉ &nbsp; View</button></td></tr>})}</tbody></table></div><footer><span>Showing all {filtered.length} drivers · highest IADC to lowest IADC</span></footer></article>
