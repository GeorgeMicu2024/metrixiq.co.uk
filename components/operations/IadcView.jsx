...
export default function IadcView({organizationId,onOpenDriver,onImport,siteFilter="all"}){
  ...
  const pageSize=10,pages=Math.max(1,Math.ceil(filtered.length/pageSize)),safePage=Math.min(page,pages),shown=filtered.slice((safePage-1)*pageSize,safePage*pageSize);
  ...
  return <div className="iadcv3">
    ...
    <section className="iadcv3-kpis">
      ...
      <article className="mint"><i>✓</i><div><span>DWC (Average)</span><strong>{pct(dwcAvg,1)}</strong><small>{dwcAvg==null?"Awaiting DWC evidence":"Workflow compliance"}</small></div></article>
    ...
    <section className="iadcv3-charts">
      ...
      <article className="panel"><div className="panel-head"><div><h2>DWC Overview</h2><p>Driver Workflow Compliance</p></div></div><div className="iadcv3-donut-row"><div className="iadcv3-donut dwc"><div><strong>{pct(dwcAvg,1)}</strong><span>DWC</span></div></div><div className="iadcv3-legend">{dwcErrors.length?dwcErrors.map((x,i)=><p key={x.key}><i className={"dwc"+i}/><span>{x.label}</span><b>{x.value}</b></p>):<p className="empty">Import a report containing DWC error categories to populate the breakdown.</p>}</div></div></article>
    </section>

    <section className="iadcv3-lower">
      <article className="panel iadcv3-table"><div className="panel-head"><div><h2>Driver Performance</h2><p>Click a driver to view detailed breakdown</p></div></div><div className="table-wrap"><table><thead><tr><th>#</th><th>Driver name</th><th>Transporter ID</th><th>IADC %</th><th>Band</th><th>DWC %</th><th>Actions</th></tr></thead><tbody>{shown.map((r,i)=>{const v=Number(r.iadc),b=band(v);return <tr key={r.driver_id+"-"+i} className={active===r?"active":""} onClick={()=>setDetail(r)}><td><span className="rank-badge">{(safePage-1)*pageSize+i+1}</span></td><td><b>{dname(r.drivers)}</b></td><td><code>{trid(r.drivers)}</code></td><td><strong className={"iadcv3-score "+b}>{pct(v)}</strong></td><td><span className={"iadcv3-band "+b}>{bandLabel(v)}</span></td><td><b>{pct(dwcOf(r))}</b></td><td><button onClick={e=>{e.stopPropagation();setDetail(r)}}>◉ &nbsp; View</button></td></tr>})}</tbody></table></div><footer><span>Showing {(safePage-1)*pageSize+1}–{Math.min(safePage*pageSize,filtered.length)} of {filtered.length} drivers</span><div>{Array.from({length:Math.min(5,pages)},(_,i)=>i+1).map(p=><button className={p===safePage?"active":""} key={p} onClick={()=>setPage(p)}>{p}</button>)}</div></footer></article>
