...
")],{type:"text/csv"});const u=URL.createObjectURL(blob),a=document.createElement("a");a.href=u;a.download=`metrixiq-${metric}-${mode==="daily"?selectedDay:selectedWeek}.csv`;a.click();setTimeout(()=>URL.revokeObjectURL(u),300)};
  ...
  return <div className="iadcv3">
    ...
    {metric==="iadc"?<section className="iadcv3-import-panel"><div className="iadcv3-tabs"><button className={mode==="daily"?"active":""} onClick={()=>setMode("daily")}>Daily</button><button className={mode==="weekly"?"active":""} onClick={()=>setMode("weekly")}>Weekly</button></div><div className="iadcv3-period">{mode==="daily"?<select value={selectedDay} onChange={e=>setDay(e.target.value)}>{days.length?days.map(d=><option key={d}>{d}</option>):<option value="">No daily report</option>}</select>:<select value={selectedWeek} onChange={e=>setWeek(e.target.value)}>{weeks.length?weeks.map(w=><option key={w}>{w}</option>):<option value="">No weekly report</option>}</select>}</div><div className="iadcv3-dropzone"><input ref={fileInput} type="file" hidden accept={IADC_ACCEPT} onChange={e=>quickImport(e.target.files?.[0])}/><div><strong>⇧ Upload IADC Report</strong><span>Amazon IADC report · .xlsx, .xls, .html, .htm or .pdf</span></div><button className="btn primary" disabled={importing} onClick={()=>fileInput.current?.click()}>{importing?"Importing…":"Choose File"}</button></div><div className="iadcv3-import-actions"><input aria-label="Search IADC drivers" value={query} onChange={e=>setQuery(e.target.value)} placeholder="⌕  Search driver…"/><select className="iadcv3-sort" aria-label="Sort IADC score" value={sortDir} onChange={e=>setSortDir(e.target.value)}><option value="desc">IADC ↓ High to Low</option><option value="asc">IADC ↑ Low to High</option></select><button className="btn ghost" onClick={()=>setEditMode(v=>!v)}>{editMode?"Done":"✎  Edit"}</button><button className="btn ghost danger" onClick={reset}>↻  Clear</button><button className="btn ghost" onClick={exportCsv}>⇩  Export</button></div></section>:<section className="iadcv3-toolbar"><div className="iadcv3-tabs"><button className={mode==="daily"?"active":""} onClick={()=>setMode("daily")}>Daily</button><button className={mode==="weekly"?"active":""} onClick={()=>setMode("weekly")}>Weekly</button></div>{mode==="daily"?<select value={selectedDay} onChange={e=>setDay(e.target.value)}>{days.map(d=><option key={d}>{d}</option>)}</select>:<select value={selectedWeek} onChange={e=>setWeek(e.target.value)}>{weeks.map(w=><option key={w}>{w}</option>)}</select>}<input aria-label={`Search ${meta.label} drivers`} value={query} onChange={e=>setQuery(e.target.value)} placeholder="⌕  Search driver…"/><button className="iadcv3-reset" onClick={reset}>Reset</button></section>}
    ...
    <section className="iadcv3-lower">
      ...
      {(()=>{const dwcRows=selected.filter(r=>dwcOf(r)!=null).filter(r=>dname(r.drivers).toLowerCase().includes(query.toLowerCase())).sort((a,b)=>sortDir==="asc"?Number(dwcOf(a))-Number(dwcOf(b)):Number(dwcOf(b))-Number(dwcOf(a)));const compliant=dwcRows.filter(r=>Number(dwcOf(r))>=85).length;const below=dwcRows.length-compliant;const dwcActive=(detail&&dwcOf(detail)!=null)?detail:(dwcRows[0]||null);const issues=r=>Object.entries(errorLabels).filter(([k])=>Number(r.raw_data?.dwc_detail?.errors?.[k]||0)>0).map(([,l])=>l);return <>
