...
export default function DashboardClient() {
  ...
  <div className="topbar-controls">
    ...
    <button className="topbar-notify" onClick={()=>navigate("notifications")} aria-label="Notifications">♢<i/></button>
    <button className="topbar-profile" onClick={()=>navigate("settings")}><b>{initials(session?.name||session?.email||"M")}</b><span><strong>{session?.name||"Manager"}</strong><small>{session?.role||"Manager"}</small></span><em>⌄</em></button>
  </div>
