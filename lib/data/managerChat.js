export async function fetchManagerChatMessages(supabase, organizationId, site = null, limit = 100) {
  ...
  let names = {};
  if (ids.length) {
    const { data: profiles } = await supabase.from("profiles").select("id,full_name,email,last_login_at").in("id", ids);
...
export async function fetchChatPins(supabase,organizationId){const {data,error}=await supabase.from("manager_chat_pins").select("id,message_id,pinned_by,created_at").eq("organization_id",organizationId);if(error)throw error;return data||[]}
export async function toggleChatPin(supabase,{organizationId,messageId,userId,pinned}){if(pinned){const {error}=await supabase.from("manager_chat_pins").delete().eq("message_id",messageId);if(error)throw error}else{const {error}=await supabase.from("manager_chat_pins").insert({organization_id:organizationId,message_id:messageId,pinned_by:userId});if(error)throw error}}

export async function searchChatMessages(supabase,{organizationId,site=null,query,limit=50}){let q=supabase.from("manager_chat_messages").select("id,site,sender_id,body,created_at,deleted_at").eq("organization_id",organizationId).is("deleted_at",null).ilike("body",`%${String(query||"").trim()}%`).order("created_at",{ascending:false}).limit(limit);q=site?q.eq("site",site):q.is("site",null);const {data,error}=await q;if(error)throw error;return data||[]}
export async function fetchChatPins(supabase,organizationId,messageIds=[]){if(!messageIds.length)return[];const {data,error}=await supabase.from("manager_chat_pins").select("*").eq("organization_id",organizationId).in("message_id",messageIds);if(error)throw error;return data||[]}
export async function toggleChatPin(supabase,{organizationId,messageId,userId,active}){if(active){const {error}=await supabase.from("manager_chat_pins").delete().eq("message_id",messageId);if(error)throw error}else{const {error}=await supabase.from("manager_chat_pins").insert({organization_id:organizationId,message_id:messageId,pinned_by:userId});if(error)throw error}}

export async function searchChatMessages(supabase,organizationId,site,term){let q=supabase.from("manager_chat_messages").select("id,sender_id,body,created_at,site,attachment_name").eq("organization_id",organizationId).is("deleted_at",null).ilike("body",`%${term}%`).order("created_at",{ascending:false}).limit(50);q=site?q.eq("site",site):q.is("site",null);const {data,error}=await q;if(error)throw error;return data||[]}
export async function fetchChatPins(supabase,organizationId){const {data,error}=await supabase.from("manager_chat_pins").select("id,message_id,pinned_by,created_at").eq("organization_id",organizationId).order("created_at",{ascending:false});if(error)throw error;return data||[]}
export async function toggleChatPin(supabase,{organizationId,messageId,userId,active}){if(active){const {error}=await supabase.from("manager_chat_pins").delete().eq("message_id",messageId);if(error)throw error}else{const {error}=await supabase.from("manager_chat_pins").insert({organization_id:organizationId,message_id:messageId,pinned_by:userId});if(error)throw error}}

export async function fetchChatPins(supabase,organizationId){const {data,error}=await supabase.from("manager_chat_pins").select("message_id,pinned_by,created_at").eq("organization_id",organizationId);if(error)throw error;return data||[]}
export async function toggleChatPin(supabase,{organizationId,messageId,userId,pinned}){if(pinned){const {error}=await supabase.from("manager_chat_pins").delete().eq("message_id",messageId);if(error)throw error}else{const {error}=await supabase.from("manager_chat_pins").insert({organization_id:organizationId,message_id:messageId,pinned_by:userId});if(error)throw error}}
