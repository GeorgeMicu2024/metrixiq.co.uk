...
export async function sendManagerChatMessage(supabase, { organizationId, site = null, body }) {
  ...
  const { data, error } = await supabase.from("manager_chat_messages").insert({ organization_id: organizationId, site: site || null, sender_id: userData.user.id, body: String(body || "").trim(), reply_to_id: arguments[1]?.replyToId || null }).select("id").single();
  if (error) throw error;
  return data;
}
