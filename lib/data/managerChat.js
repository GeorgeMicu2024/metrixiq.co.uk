export async function fetchManagerChatMessages(supabase, organizationId, site = null, limit = 100) {
  let query = supabase.from("manager_chat_messages").select("id,organization_id,site,sender_id,body,created_at,edited_at,reply_to_id,deleted_at").eq("organization_id", organizationId).order("created_at", { ascending: false }).limit(limit);
...
export async function sendManagerChatMessage(supabase, { organizationId, site = null, body, replyToId = null }) {
  const { data: userData, error: userError } = await supabase.auth.getUser();
  if (userError || !userData.user) throw userError || new Error("Authentication required.");
  const { data, error } = await supabase.from("manager_chat_messages").insert({ organization_id: organizationId, site: site || null, sender_id: userData.user.id, body: String(body || "").trim(), reply_to_id: replyToId }).select("id").single();
  if (error) throw error;
  return data;
}
