...
export async function resolveMentorMapping(supabase, organizationId, record, driverId) {
  const { data, error } = await supabase.rpc("resolve_unmatched_driver_record", {
    p_organization_id: organizationId,
    p_record_id: record.id,
    p_driver_id: driverId,
    p_alias_name: record.raw_name || "",
    p_normalized_name: record.normalized_name || "",
    p_name_signature: record.payload?.driver?.nameSignature || record.normalized_name || "",
  });
  if (error) throw error;
  return data;
}
