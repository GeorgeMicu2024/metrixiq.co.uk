...
export async function loadWorkspaceContext(supabase, user) {
  const { error: inviteError } = await supabase.rpc("redeem_my_pending_invites");
  if (inviteError) throw inviteError;

  const [{ data: profile, error: profileError }, resolved] = await Promise.all([
  ...
  const { error: touchError } = await supabase.rpc("touch_last_login");
