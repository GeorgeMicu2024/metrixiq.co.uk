...
export async function fetchDriver360V2Data(supabase, organizationId, driverId) {
  const [
    ...
    supabase
      ...
      .limit(200),
    supabase.rpc("list_driver_notes", {
    ...
    supabase
      .from("audit_events")
      .select("id,actor_id,event_type,entity_type,entity_id,driver_id,site,week_label,action,before_data,after_data,metadata,created_at")
      .eq("organization_id", organizationId)
      .eq("driver_id", driverId)
      .order("created_at", { ascending: false })
      .limit(500),
...
export async function fetchIncidentCenterData(supabase, organizationId) {
  const [incidents, feedback, assignees] = await Promise.all([
    ...
    supabase
      ...
      .order("feedback_date", { ascending: false })
      .limit(2000),
    supabase.rpc("list_incident_assignees", { p_organization_id: organizationId }),
  ]);
