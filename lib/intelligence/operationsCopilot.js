...
import { recommendLearnedIntervention } from "./decisionEngine.js";
...
export function explainDriverRisk(driver={},interventionHistory=[]){
  ...
  const defaultAction=primary?.key==="concessions"?"Review evidence and root cause":primary?.key==="mentor"?"Review eMentor events and coach the primary behaviour":primary?"Open targeted coaching against the primary KPI":"Monitor next reporting cycle";
  const learned=primary?recommendLearnedIntervention(primary.key,interventionHistory):null;
  const nextAction=learned?.status==="learned"&&learned.recommendation?learned.recommendation:defaultAction;
  const learningExplanation=learned?.status==="learned"?` Historical evidence: ${learned.explanation} Confidence ${learned.confidence}%.`:"";
  return {primary,factors:factors.slice(0,4),nextAction,defaultAction,learnedRecommendation:learned,explanation:primary?`${primary.reason}. ${nextAction}.${learningExplanation}`:"No material KPI exception is visible in current evidence."};
}

export function buildInterventionQueue(drivers=[],interventionHistory=[]){
  return drivers.map(driver=>({driver,...explainDriverRisk(driver,interventionHistory)}))
    .filter(x=>x.primary)
