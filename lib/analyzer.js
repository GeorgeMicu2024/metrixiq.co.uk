...
function parseConcessionMatrix(matrix, fileName, sheetName) {
  const looksConcession =
    /concession/i.test(fileName) ||
    matrix.some((row) =>
      row.some((cell) => /concession/i.test(String(cell)))
    );
  ...
  for (let h = 0; h < Math.min(matrix.length, 50); h++) {
    ...
    const total = exactIndex(headers, totalAliases);
    const dnr = exactIndex(headers, dnrAliases);
    const rts = exactIndex(headers, rtsAliases);
    const tracking = exactIndex(headers, trackingAliases);

    const hasIdentity = id >= 0 || name >= 0;

    let priority = 0;

    if (hasIdentity && total >= 0) {
      priority = 120;
    } else if (hasIdentity && (dnr >= 0 || rts >= 0)) {
      priority = 110;
    } else if (hasIdentity && tracking >= 0) {
      priority = 80;
    }
  ...
  for (const cells of matrix.slice(best.h + 1)) {
    ...
    if (best.total >= 0) {
      ...
      const dnrValue =
        best.dnr >= 0
          ? numeric(cells[best.dnr], "concessions")
          : null;

      const rtsValue =
        best.rts >= 0
          ? numeric(cells[best.rts], "concessions")
          : null;

      if (dnrValue != null || rtsValue != null) {
        current.dnr += dnrValue ?? 0;
        current.rts += rtsValue ?? 0;
