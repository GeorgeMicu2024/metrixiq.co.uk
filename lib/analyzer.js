...
function inferSiteCode(fileName, fallbackText = "") {
...
function targetPerformance(metrics) {
  const values = [];
  if (metrics.dcr != null) values.push(Math.min(105, metrics.dcr / TARGETS.dcr * 100));
  if (metrics.pod != null) values.push(Math.min(105, metrics.pod / TARGETS.pod * 100));
  if (metrics.iadc != null) values.push(Math.min(105, metrics.iadc / TARGETS.iadc * 100));
  if (metrics.mentor_score != null) values.push(Math.min(105, metrics.mentor_score / TARGETS.mentor * 100));
...
function issueFor(metrics) {
  if (metrics.dcr != null && metrics.dcr < TARGETS.dcr) return `DCR below ${TARGETS.dcr.toFixed(2)}% target`;
  if (metrics.pod != null && metrics.pod < TARGETS.pod) return `POD below ${TARGETS.pod.toFixed(2)}% target`;
  if (metrics.iadc != null && metrics.iadc < TARGETS.iadc) return `IADC below ${TARGETS.iadc}% target`;
  if (metrics.mentor_score != null && metrics.mentor_score < TARGETS.mentor) return `Mentor score below ${TARGETS.mentor}`;
  if (metrics.cc != null && metrics.cc < TARGETS.cc) return `Contact Compliance below ${TARGETS.cc.toFixed(2)}% target`;
