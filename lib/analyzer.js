...
export async function analyseFiles(files) {
  ...
  for (const file of files) {
    ...
    try {
      ...
      for (const output of outputs) {
        ...
        if (output.siteScorecard) siteScorecards.push({ ...output.siteScorecard, period: output.period || filePeriod });

        for (const record of output.records || []) {
          pendingRecords.push({ record, period: record.period || output.period || filePeriod });
        }
      }
  ...
  for (const item of pendingRecords) {
    ...
    const bucketKey = period.granularity === "daily" ? period.key : period.key;
  ...
  const periods = [...periodBuckets.values()].map((bucket) => {
    ...
    return {
      ...
      granularity: bucket.granularity || "weekly",
      year: bucket.year,
      week: bucket.week,
