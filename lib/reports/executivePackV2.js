...
import { buildReportFreshness, predictDeterioration, coachingEffectiveness, buildWeeklyExecutiveAi, buildOutcomeMonitor } from "../intelligence/decisionEngine.js";
...
export function buildExecutivePack(data = {}, {
  ...
  const freshness = buildReportFreshness(data.imports || []);
  const predictions = predictDeterioration(allScopedRows);
  const coachingImpact = coachingEffectiveness(coaching, allScopedRows);
  const outcomeMonitor = buildOutcomeMonitor(tasks, allScopedRows);
  const aiExecutive = buildWeeklyExecutiveAi({
    intelligence: { highRisk: fairPoor.length, confidence: Math.max(0, 100 - unmatched.length * 2 - failedImports.length * 5) },
    freshness,
    predictions,
  });
  ...
  return {
    ...
    actions,
    ai: { executive: aiExecutive, freshness, predictions: predictions.slice(0,20), coachingEffectiveness: coachingImpact, outcomes: outcomeMonitor },
  };
}
