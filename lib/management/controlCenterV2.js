...
export function buildManagerWorkQueue({
  ...
  const smart = latest.flatMap(metricSignals).filter((item) => !closedTaskKeys.has(item.id));
  const alertItems = (alerts || [])
    .filter((item) => item.status !== "resolved")
    .map(alertSignal)
    .filter((item) => !closedTaskKeys.has(item.id));
  const coachingItems = (coachingCases || [])
    .map(coachingSignal)
    .filter(Boolean)
    .filter((item) => !closedTaskKeys.has(item.id));
  ...
  if (unmatchedCount > 0 && !closedTaskKeys.has("workspace:unmatched")) {
  ...
  if (failedImports > 0 && !closedTaskKeys.has("workspace:failed-imports")) {
    workspaceItems.push(signal({}, {
