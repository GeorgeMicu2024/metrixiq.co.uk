...
export async function persistImportAudit(
  ...
  const importContext = analysis?.importContext || {};

  const importRows = (analysis?.fileResults || []).map((result) => {
    const file = fileMap.get(result.name);
    const dailyReportDate =
      importContext.type === "mentor-daily" ? importContext.reportDate : null;
    ...
    return {
      ...
      period_start: dailyReportDate || result.period?.periodStart || null,
      period_end: dailyReportDate || result.period?.periodEnd || null,
      error_message:
        result.error ||
      ...
      metadata: {
        ...
        week_label: result.period?.weekLabel || importContext.weekLabel || null,
        import_mode: importContext.type || "standard",
        report_date: dailyReportDate || null,
        target_week: importContext.targetWeek || null,
        activity_site: site || null,
      },
