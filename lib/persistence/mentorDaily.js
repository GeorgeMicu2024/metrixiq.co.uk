...
export async function persistMentorDailyAnalysis({
  ...
  for (const period of analysis?.periods || []) {
    ...
    for (const driver of period.drivers || []) {
      ...
      const sourceIdentityKey = String(
        driver.mentorHash ||
          driver.details?.mentor?.identityKey ||
          (isValidTrid(driver.id) ? normalizeTrid(driver.id) : "") ||
          resolved.driver.id
      ).trim();
      ...
      const row = {
        ...
        raw_data: {
          ...
          report_date: reportDate,
          import_mode: "daily",
          activity_site: resolvedSite,
        },
      };

      rowsBySource.set(sourceIdentityKey, row);
    }
  }
  ...
  if (rows.length) {
    const { data, error } = await supabase
      ...
      .upsert(rows, {
        onConflict: "organization_id,site,report_date,source_identity_key",
      })
      .select("id");
  ...
  return {
    ...
    reconciliation: {
      ...
      skippedWithoutScore,
      accountedRows: rows.length + unmatchedCount + skippedWithoutScore,
      balanced: sourceRows === rows.length + unmatchedCount + skippedWithoutScore,
    },
  };
}
