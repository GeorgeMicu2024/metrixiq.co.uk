...
export async function persistMentorHashAliases(
  ...
  for (const alias of mentorAliases) {
    ...
    rowsByKey.set(key, {
      ...
      source: `${alias.source || "eMentor alias master"}${alias.site ? ` [site:${String(alias.site).trim().toUpperCase()}]` : ""}`,
    });
  }
...
export async function storeUnmatched(supabase, organizationId, rows) {
  ...
  const prepared = rows.map((row) => {
    if (row.report_type !== "mentor_daily") return row;

    const reconciliationKey = String(
    ...
    return {
      ...row,
      reconciliation_key: reconciliationKey || null,
    };
  });

  const mentorRows = prepared.filter(
    (row) => row.report_type === "mentor_daily" && row.reconciliation_key
  );
  const otherRows = prepared.filter(
    (row) => row.report_type !== "mentor_daily" || !row.reconciliation_key
  );
  ...
  if (mentorRows.length) {
    // PostgREST onConflict accepts column names, not expression indexes such as
    // payload->>'reportDate'. De-duplicate against the existing reconciliation
    // rows first, then insert only identities that are not already represented.
    // week_label is retained as the report-period discriminator while
    // reconciliation_key preserves distinct eMentor source accounts.
    const weekLabels = [...new Set(mentorRows.map((row) => row.week_label).filter(Boolean))];
    let existingRows = [];

    if (weekLabels.length) {
      const { data, error: existingError } = await supabase
        .from("unmatched_driver_records")
        .select("week_label,reconciliation_key")
        .eq("organization_id", organizationId)
    ...
    const existingKeys = new Set(
      existingRows.map((row) => `${row.week_label || ""}::${row.reconciliation_key || ""}`)
    );
    const pendingByKey = new Map();

    for (const row of mentorRows) {
      const key = `${row.week_label || ""}::${row.reconciliation_key || ""}`;
      if (!existingKeys.has(key)) pendingByKey.set(key, row);
    }
