...
export function parseMentorAliasMatrix(matrix, fileName, sheetName) {
  if (!matrix?.length) return null;

  const performanceHeader = matrix.slice(0, Math.min(matrix.length, 20)).some((row) => {
    const headers = (row || []).map(clean);
    const hasScore = headers.some((value) =>
      value === "score" ||
      value.includes("fico") ||
      value.includes("mentor score") ||
      value.includes("safe driving score")
    );
    const behaviourSignals = headers.filter((value) =>
      /acceleration|braking|cornering|distraction|speeding|training/.test(value)
    ).length;
    return hasScore && behaviourSignals >= 1;
  });
  if (performanceHeader) return null;

  const looksClearNamePart = (value) => {
    const text = String(value ?? "").trim();
    if (!text || looksEncryptedNameToken(text) || /\d/.test(text)) return false;
    if (/\b(low|medium|high)\s+risk\b/i.test(text)) return false;
    return /^[A-Za-zÀ-ž'’ .-]{2,}$/.test(text);
  };
  ...
  if (headerIndex < 0) {
    ...
    const valid = sample.filter((cells) =>
      looksEncryptedNameToken(cells?.[0]) &&
      looksEncryptedNameToken(cells?.[1]) &&
      looksClearNamePart(cells?.[2]) &&
      looksClearNamePart(cells?.[3])
    );
  ...
  for (const cells of matrix.slice(headerIndex >= 0 ? headerIndex + 1 : 0)) {
    ...
    if (
      !looksEncryptedNameToken(firstHash) ||
      !looksEncryptedNameToken(lastHash) ||
      !looksClearNamePart(firstName) ||
      !looksClearNamePart(lastName) ||
      !isUsablePersonName(name) ||
      !key ||
      seen.has(key)
    ) continue;
    seen.add(key);
...
export function parseMentorMatrix(matrix, fileName, sheetName) {
  if (!matrix?.length) return null;
  ...
  const station = fuzzyIndex(headers, ["station", "site", "delivery station"]);
  const totalTrips = fuzzyIndex(headers, ["total driver trips", "driver trips", "total trips", "trips"]);
  const records = [];
  const identities = [];
  ...
  for (const cells of matrix.slice(best.h + 1)) {
    ...
    // eMentor/FICO Safe Driving Score is a dedicated score, not a generic
    // percentage/count. Reject values outside the plausible product scale so
    // a misidentified column cannot contaminate fleet/site averages.
    const mentorScore = mentor != null && mentor >= 0 && mentor <= 1000 ? mentor : null;
    ...
    if (mentorScore == null || (!usableName && !isValidTrid(trid) && !identityHint?.key)) continue;
    if (isValidTrid(trid) && usableName) {
      identities.push({ trid, name: combinedName, source: fileName, confidence: 1 });
    }

    const rowSite = station >= 0 ? normalizeSiteCode(cells[station]) : inferSiteCode(fileName, sheetName);
    const trips = totalTrips >= 0 ? numeric(cells[totalTrips], "trips") : null;
    ...
    records.push(makeRecord({
      ...
      site: rowSite,
      metrics: { mentor_score: mentorScore },
      priorities: { mentor_score: 100 },
      identityHint,
      details: {
        mentor: {
          totalTrips: trips,
          station: rowSite || null,
...
export function parseScorecardMatrix(matrix, fileName, sheetName) {
  if (!matrix?.length) return null;
  ...
  for (const cells of matrix) {
    ...
    const delivered = indexes.delivered >= 0 ? numeric(cells[indexes.delivered], "delivered") : null;
    const parsedDcr = indexes.dcr >= 0 ? numeric(cells[indexes.dcr], "dcr") : null;
    // A 0% DCR with zero delivered packages has no delivery opportunity.
    // Store it as missing evidence instead of a genuine performance failure.
    const dcr = parsedDcr === 0 && delivered === 0 ? null : parsedDcr;
