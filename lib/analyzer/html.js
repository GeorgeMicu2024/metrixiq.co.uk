...
function parseIadcSection(section, fileName, period, site) {
...
function parseIadcHtml(doc, fileName, period) {
  ...
  if (weekSection) {
    const records = parseIadcSection(weekSection, fileName, { ...period, granularity: "weekly" }, site);
  ...
  for (const section of doc.querySelectorAll('section[data-tab^="day_"]')) {
    ...
    const records = parseIadcSection(section, fileName, dailyPeriod, site);
