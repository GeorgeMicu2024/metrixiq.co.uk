...
export function percentageMetric(value, metric = "percentage") {
  const parsed = numeric(value, metric);
  return parsed != null && parsed >= 0 && parsed <= 100 ? parsed : null;
}

export function rollingSeries(text, label, keyPattern) {
  const start = text.indexOf(label);
  if (start < 0) return {};
  const tail = text.slice(start + label.length);
  const boundary = tail.indexOf("Unable to display chart");
  const chunk = boundary >= 0 ? tail.slice(0, boundary) : tail.slice(0, 500);
  const keys = chunk.match(keyPattern) || [];
  const values = (chunk.match(/\d+(?:\.\d+)?%/g) || [])
    .map((value) => percentageMetric(value))
    .filter((value) => value != null);
  return Object.fromEntries(keys.slice(0, values.length).map((key, i) => [key, values[i]]));
}
...
function parseIadcSection(section, fileName, period, site, complianceSummary = null) {
  ...
  for (const cells of summary.rows.slice(summary.headerIndex + 1)) {
    ...
    const iadc = iadcIndex >= 0 ? percentageMetric(cells[iadcIndex], "iadc") : null;
    const dwc = dwcIndex >= 0 ? percentageMetric(cells[dwcIndex], "dwc") : null;
...
function parseContactComplianceHtml(doc, fileName, period) {
  let summary = null;
  for (const table of doc.querySelectorAll("table")) {
    const matrix = tableMatrix(table);
    for (let h = 0; h < Math.min(matrix.length, 12); h++) {
      const headers = matrix[h];
      const iTrid = exactIndex(headers, ["Transporter ID", "TRID"]);
      const iAddresses = fuzzyIndex(headers, ["Total Addresses", "Addresses"]);
      const iContacts = fuzzyIndex(headers, ["Total Contacts", "Contacts"]);
      const iCc = fuzzyIndex(headers, ["Contact Compliance", "Contact Compliance %", "CC"]);
      if (iTrid < 0 || iAddresses < 0 || iContacts < 0 || iCc < 0) continue;
      const records = [];
      for (const cells of matrix.slice(h + 1)) {
        const trid = normalizeTrid(cells[iTrid]);
        if (!isValidTrid(trid)) continue;
        const required = numeric(cells[iAddresses], "addresses");
        const completed = numeric(cells[iContacts], "contacts");
        const cc = numeric(cells[iCc], "cc");
        if (required == null && completed == null && cc == null) continue;
        records.push(makeRecord({
          trid,
          metrics: cc == null ? {} : { cc },
          priorities: cc == null ? {} : { cc: 100 },
          source: fileName,
          period: { ...period, granularity: "weekly" },
          details: { contactCompliance: { required, completed, missed: required != null && completed != null ? Math.max(0, required - completed) : null, reasons: [] } },
        }));
      }
      if (records.length) { summary = records; break; }
    }
    if (summary) break;
  }
  if (!summary) return null;

  const byTrid = new Map(summary.map(record => [record.trid, record]));
  for (const table of doc.querySelectorAll("table")) {
    const matrix = tableMatrix(table);
    for (let h = 0; h < Math.min(matrix.length, 12); h++) {
      const headers = matrix[h];
      const iTrid = exactIndex(headers, ["Transporter ID", "TRID"]);
      const iReason = fuzzyIndex(headers, ["Reason Code", "Reason"]);
      const iTracking = fuzzyIndex(headers, ["Tracking ID", "Tracking"]);
      if (iTrid < 0 || iReason < 0 || iTracking < 0) continue;
      for (const cells of matrix.slice(h + 1)) {
        const trid = normalizeTrid(cells[iTrid]);
        const record = byTrid.get(trid);
        if (!record) continue;
        const reason = String(cells[iReason] ?? "").trim();
        const trackingId = String(cells[iTracking] ?? "").trim();
        if (reason || trackingId) record.details.contactCompliance.reasons.push({ reason, trackingId });
      }
    }
  }
  return { reportType: "contact_compliance", records: summary, identities: [], rows: summary.length, period: { ...period, granularity: "weekly" } };
}
...
export async function parseHtml(file) {
  ...
  if (lower.includes("dwc-iadc") || clean(doc.body?.textContent).includes("in app delivery workflow compliance")) {
    const outputs = parseIadcHtml(doc, file.name, period);
  ...
  if (lower.includes("contact-compliance") || clean(doc.body?.textContent).includes("contact compliance")) {
    const output = parseContactComplianceHtml(doc, file.name, period);
    if (output) return [output];
  }
