...
function parseIadcHtml(doc, fileName, period) {
  ...
  for (const cells of summary.rows.slice(summary.headerIndex + 1)) {
    ...
    const dwcErrors = Object.fromEntries(Object.entries(dwcErrorIndexes).map(([key,index]) => [key,index >= 0 ? numeric(cells[index], key) : null]).filter(([,value]) => value != null));
    records.push(makeRecord({ trid, site, metrics, priorities, source: fileName, period, details: { dwc: { errors: dwcErrors } } }));
  }
