...
export function numeric(value, key) {
  ...
  if (percentMetrics.has(key)) {
    // Percentage metrics are stored canonically on a 0..100 scale.
    // Spreadsheet/HTML sources may expose them either as fractions (0.973)
    // or percentages (97.3). Reject impossible values rather than allowing
    // bad evidence to poison fleet/site KPI averages.
    if (result >= 0 && result <= 1.01) result *= 100;
...
export function scorecardTierFromTotal(value) {
