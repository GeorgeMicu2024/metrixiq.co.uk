...
function metricRowFromDriver(driver, organizationId, driverId, period, importIdByFile) {
  ...
  const row = {
    ...
    raw_data: {
      ...
      analysis_version: "professional-v6",
    },
  };
...
async function persistMentorHashAliases(supabase, organizationId, mentorAliases = [], indexes) {
  ...
  for (const alias of mentorAliases) {
    ...
    if (!key) continue;
    let resolved = null;
    if (isValidTrid(alias.trid)) resolved = resolveIdentity({ trid: alias.trid }, indexes);
    if (!resolved?.driver && isUsablePersonName(alias.name)) resolved = resolveIdentity({ name: alias.name }, indexes);
    ...
    rowsByKey.set(key, {
      ...
      confidence: Math.max(0.9, Number(alias.matchConfidence || alias.confidence || resolved.confidence || 0.9)),
  ...
  const { data, error } = await supabase
    .from("driver_aliases")
    .upsert(rows, { onConflict: "organization_id,alias_type,alias_normalized" })
    .select("id");
  if (error) throw error;
  return data?.length || 0;
}

async function saveResolvedMentorHash(supabase, organizationId, driverId, driver) {
  const key = String(driver?.mentorHash || driver?.details?.mentor?.identityKey || "").trim();
  if (!key || !driverId) return;
  const { error } = await supabase.from("driver_aliases").upsert({
    organization_id: organizationId,
    driver_id: driverId,
    alias_type: "mentor_hash",
    alias_value: key,
...
async function seedIdentityRecords(supabase, organizationId, identityRecords = [], sourceRecords = []) {
  ...
  for (const source of sourceRecords) {
    const trid = normalizeTrid(source.id ?? source.trid);
    if (!isValidTrid(trid) || !isUsablePersonName(source.name)) continue;
    identityByTrid.set(trid, {
      trid,
      name: String(source.name).trim(),
      site: source.site && source.site !== "Unknown" ? source.site : null,
      source: source.sources?.[0] || "operational report",
      confidence: 0.96,
    });
  }
  // Explicit masters always win over operational-source names.
  for (const identity of identityRecords) {
    const trid = normalizeTrid(identity.trid);
    if (!isValidTrid(trid) || !isUsablePersonName(identity.name)) continue;
