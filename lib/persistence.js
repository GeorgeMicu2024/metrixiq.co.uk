...
export async function persistAnalysisWithClient({ supabase, organizationId, analysis, files = [], site = null }) {
  ...
  for (const period of analysis?.periods || []) {
    for (const driver of period.drivers || []) {
      sourceRows += 1;
      let resolved = resolveIdentity({ trid: driver.id, name: driver.name, mentorHash: driver.mentorHash || driver?.details?.mentor?.identityKey || (String(driver.id || "").startsWith("MENTOR:") ? String(driver.id).slice(7) : null) }, indexes);
