-- Resolve an unmatched eMentor row and materialize it into Daily history immediately.
...
as $function$
declare v_row public.unmatched_driver_records%rowtype; v_driver public.drivers%rowtype; v_report_date date; v_score numeric; v_source_key text; v_name text;
begin
 ...
 v_report_date:=nullif(v_row.payload->>'reportDate','')::date; v_score:=nullif(v_row.payload->>'score','')::numeric;
 if v_report_date is null then raise exception 'eMentor source row has no report date'; end if;
 v_source_key:=coalesce(nullif(v_row.payload->'driver'->>'mentorHash',''),nullif(v_row.raw_trid,''),p_driver_id::text);
 v_name:=regexp_replace(btrim(coalesce(v_row.raw_name,'')),'\s+',' ','g');
 if v_name<>'' and nullif(v_row.normalized_name,'') is not null then
  insert into public.driver_aliases(organization_id,driver_id,alias_type,alias_value,alias_normalized,confidence,source) values(p_organization_id,p_driver_id,'name',v_name,v_row.normalized_name,1,'manual eMentor mapping')
  on conflict(organization_id,alias_type,alias_normalized) do update set driver_id=excluded.driver_id,alias_value=excluded.alias_value,confidence=1,source=excluded.source;
 end if;
 if nullif(v_row.payload->'driver'->>'mentorHash','') is not null then
  insert into public.driver_aliases(organization_id,driver_id,alias_type,alias_value,alias_normalized,confidence,source) values(p_organization_id,p_driver_id,'mentor_hash',v_row.payload->'driver'->>'mentorHash',v_row.payload->'driver'->>'mentorHash',1,'manual eMentor mapping')
  on conflict(organization_id,alias_type,alias_normalized) do update set driver_id=excluded.driver_id,alias_value=excluded.alias_value,confidence=1,source=excluded.source;
 end if;
 ...
 on conflict(organization_id,report_date,source_identity_key) do update set driver_id=excluded.driver_id,source_import_id=excluded.source_import_id,week_label=excluded.week_label,mentor_score=excluded.mentor_score,raw_data=excluded.raw_data;
 update public.unmatched_driver_records set status='resolved',matched_driver_id=p_driver_id,resolved_at=now() where id=p_record_id and organization_id=p_organization_id;
