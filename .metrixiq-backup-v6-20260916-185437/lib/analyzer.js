...
function parseMentorMatrix(matrix, fileName, sheetName) {
  ...
  const idAliases = [
    "trid", "transporter id", "transporterid", "driver id", "delivery associate id",
  ...
  for (let h = 0; h < Math.min(matrix.length, 100); h++) {
    ...
    const acceleration = fuzzyIndex(row, ["acceleration", "harsh acceleration"]);
    const braking = fuzzyIndex(row, ["braking", "harsh braking"]);
    const cornering = fuzzyIndex(row, ["cornering", "harsh cornering"]);
    const distraction = fuzzyIndex(row, ["distraction", "phone distraction", "cell phone distraction"]);
    ...
    const sourceLooksMentor = /mentor|ementor|e-mentor|edriving|driver.?report|safety/i.test(`${fileName} ${sheetName}`);
    let evidence = 0;
    if (sourceLooksMentor) evidence += 3;
    if (behaviourSignals >= 1) evidence += 2;
    if (behaviourSignals >= 3) evidence += 2;
    if (id >= 0) evidence += 1;
    if (clean(row[score]).includes("mentor") || clean(row[score]).includes("fico")) evidence += 3;
  ...
  for (const cells of matrix.slice(best.h + 1)) {
    const firstName = best.first >= 0 ? String(cells[best.first] ?? "").trim() : "";
    const lastName = best.last >= 0 ? String(cells[best.last] ?? "").trim() : "";
    ...
    if (mentor == null || (!isUsablePersonName(combinedName) && !isValidTrid(trid))) continue;
    if (isValidTrid(trid) && isUsablePersonName(combinedName)) {
...
function parseIdentityMasterMatrix(matrix, fileName, sheetName) {
  ...
  for (const cells of matrix.slice(best.h + 1)) {
    ...
    const site = best.site >= 0 ? String(cells[best.site] ?? "").trim() : "";
