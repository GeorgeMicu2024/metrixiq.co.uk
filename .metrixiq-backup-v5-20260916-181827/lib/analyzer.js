...
import { isUsablePersonName, isValidTrid, nameSignature, normalizeName, normalizeTrid } from "./identity";
...
function inferPeriod(fileName, fallbackText = "") {
  ...
  match = file.match(/\b(20\d{2})[\-_](?:w)?(\d{1,2})(?![\-_]\d{1,2})\b/i);
  if (match) {
    const year = Number(match[1]), week = Number(match[2]);
    if (week >= 1 && week <= 53) return isoWeekDetails(year, week);
  }

  match = file.match(/\b(20\d{2})[\-_](\d{2})[\-_](\d{2})\b/);
  if (match) {
...
function makeRecord({ trid = "", name = "", site = "", metrics = {}, details = {}, priorities = {}, source = "" }) {
  return {
...
function parseMentorMatrix(matrix, fileName, sheetName) {
  const headerIndex = matrix.findIndex((row) => {
    ...
    return c.includes("first name") && c.includes("last name") && c.includes("score") &&
      (c.includes("acceleration") || c.includes("braking") || c.includes("cornering"));
  });
  if (headerIndex < 0) return null;

  const headers = matrix[headerIndex];
  const first = exactIndex(headers, ["first name"]);
  const last = exactIndex(headers, ["last name"]);
  const score = exactIndex(headers, ["score", "mentor score", "ementor score"]);
  const acceleration = exactIndex(headers, ["acceleration"]);
  const braking = exactIndex(headers, ["braking"]);
  const cornering = exactIndex(headers, ["cornering"]);
  const distraction = exactIndex(headers, ["distraction"]);
  const speeding = headers.map((h, i) => clean(h) === "speeding" ? i : -1).filter((i) => i >= 0);
  const training = exactIndex(headers, ["training"]);
  const completed = exactIndex(headers, ["completed"]);

  const records = [];
  for (const cells of matrix.slice(headerIndex + 1)) {
    const name = `${cells[first] ?? ""} ${cells[last] ?? ""}`.replace(/\s+/g, " ").trim();
    const mentor = numeric(cells[score], "mentor_score");
    if (!isUsablePersonName(name) || mentor == null) continue;
    records.push(makeRecord({
      ...
      details: {
        mentor: {
          acceleration: acceleration >= 0 ? String(cells[acceleration] ?? "").trim() : "",
          braking: braking >= 0 ? String(cells[braking] ?? "").trim() : "",
          cornering: cornering >= 0 ? String(cells[cornering] ?? "").trim() : "",
          distraction: distraction >= 0 ? String(cells[distraction] ?? "").trim() : "",
          speedingRisk: speeding[0] != null ? String(cells[speeding[0]] ?? "").trim() : "",
          speedingEvents: speeding[1] != null ? numeric(cells[speeding[1]], "speeding") : null,
...
function parseGenericMatrix(matrix, fileName, sheetName) {
  ...
  const idAliases = ["trid", "transporter id", "transporterid", "driver id", "associate id", "delivery associate id", "da id", "courier id"];
  const nameAliases = ["driver name", "delivery associate name", "full name", "delivery associate", "da name", "associate", "courier"];
  const siteAliases = ["site", "station", "depot", "delivery station"];

  let headerIndex = -1;
  for (let i = 0; i < Math.min(matrix.length, 40); i++) {
    const row = matrix[i] || [];
    if (fuzzyIndex(row, idAliases) >= 0 || fuzzyIndex(row, nameAliases) >= 0) {
      headerIndex = i;
      break;
    }
  }
  ...
  const nameIndex = fuzzyIndex(headers, nameAliases);
  ...
  const reportType = /schedule|roster|master|driver.?list/.test(lower) && !records.length ? "identity_master" : "spreadsheet";
...
function parseIadcHtml(doc, fileName, period) {
  ...
  for (const cells of summary.rows.slice(summary.headerIndex + 1)) {
    ...
    records.push(makeRecord({ trid, metrics, priorities, source: fileName }));
  }
  return records.length ? { reportType: "iadc", records, identities: [], rows: records.length } : null;
}
...
function mergeDriverRecord(map, record) {
  ...
  const key = isValidTrid(trid) ? trid : isUsablePersonName(record.name) ? `NAME:${nameSignature(record.name)}` : null;
  if (!key) return;
  ...
  for (const [metric, value] of Object.entries(record.metrics || {})) {
    ...
    current.selected[metric] = metricWinner(current.selected[metric], value, priority, record.source);
  }
...
function finalDriver(record) {
  const rawMetrics = {};
  Object.entries(record.selected || {}).forEach(([metric, selected]) => { rawMetrics[metric] = selected.value; });
...
function mergeIdentityRecords(records) {
  ...
  for (const record of records) {
    ...
    const existing = byTrid.get(trid);
    if (!existing || (record.confidence ?? 0) >= (existing.confidence ?? 0)) byTrid.set(trid, { ...record, trid });
  }
  return [...byTrid.values()];
}
...
export async function analyseFiles(files) {
  ...
  for (const file of files) {
    const period = inferPeriod(file.name);
    try {
      const outputs = await parseFile(file);
      if (outputs === null) {
        fileResults.push({ name: file.name, type: file.name.split(".").pop()?.toLowerCase() || "unknown", status: "unsupported", recognized: false, rows: 0, period });
        continue;
      }
      ...
      for (const output of outputs) {
        ...
        recognized ||= !!(output.rows || output.siteScorecard || output.feedbackEvents?.length);
        identityRecords.push(...(output.identities || []));
        feedbackEvents.push(...(output.feedbackEvents || []));
        if (output.siteScorecard) siteScorecards.push({ ...output.siteScorecard, period });
        ...
        if (output.records?.length) {
          ...
          bucket.sourceFiles.add(file.name);
          for (const record of output.records) mergeDriverRecord(bucket.records, record);
        }
      }
